...
#	NLD_Functions.Parameter_Space_Plot(file_name,Option_Dictionary)
#	return
...
def main():
	...
	def parse_command_line(argvs):
		...
		parser.add_argument("-p","--plot",help=Simple_Plot.__doc__, metavar="FILE")

		parser.add_argument("-s","--splot",help= Spherical_Plot.__doc__, metavar="FILE")

		parser.add_argument("-a", "--alpha",help = Alpha_Plot.__doc__, metavar="FILE")

		parser.add_argument("-3", "--threeDplot", help = ThreeD_Plot_Cartesian.__doc__ ) 

		parser.add_argument("-g", "--angle_space", help = Angle_Space_Plot.__doc__ )

		parser.add_argument("-l", "--plot_beta_transform", help = Simple_Plot_Transform.__doc__ )

		parser.add_argument("-z", "--splot_beta_transform", help = Spherical_Plot_Transform.__doc__ )
	...
	if arguments.opts : Option_Dictionary = Create_Option_Dictionary(arguments.opts)
	else : Option_Dictionary = {}

	# Add the verbosity to the Option Dictionary
	Option_Dictionary['verbose'] = arguments.verbose

	if arguments.plot: Simple_Plot(arguments.plot,Option_Dictionary)

	if arguments.splot: Spherical_Plot(arguments.splot,Option_Dictionary)

	if arguments.alpha : Alpha_Plot(arguments.alpha,Option_Dictionary)

	if arguments.threeDplot :  ThreeD_Plot_Cartesian(arguments.threeDplot,Option_Dictionary)
	
	if arguments.angle_space : Angle_Space_Plot(arguments.angle_space,Option_Dictionary)

	if arguments.plot_beta_transform: Simple_Plot_Transform(arguments.plot_beta_transform,Option_Dictionary)

	if arguments.splot_beta_transform: Spherical_Plot_Transform(arguments.splot_beta_transform,Option_Dictionary)
	
	# These functions are found in the lib as it is getting crowded in here
