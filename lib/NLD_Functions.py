...
def Param_Space_Plot():
	from pylab import sin,cos
	
	# In this case we set the opts argument to accept 1 argument for the maximal alowed length (from the begining) of the file to plot
	if options.opts:
		max_length = int(options.opts)
	else : max_length = None

	file_name = options.param_space_plot
	(time,x,y,z,params)=Import_Data(file_name,max_length)


	# Transform to spherical polar coordinates
	(omega,a,phi) = Transform_Cartesian_2_Spherical_Radians(x,y,z)

	# Fix phi
	phi = Fix_Phi_Radians(phi,1.2)

	# Calculate the differentials from Goldreich equations
	# Import data from params
	eps_I=float(params[2].split("=")[1])
	eps_A=float(params[1].split("=")[1])
	chi=float(params[0].split("=")[1])

	# Calculate some constants
	eps_I_prime=eps_I/(1+eps_I)
	...
	for i in range(len(omega)):
		...
		omega_dot.append( Lambda*eps_A*pow(omega[i],3)*(Tsdotomega-eps_I_prime*Tsz*cosa) - eps_A*eps_I_prime*pow(omega[i],2)*Taz*cosa)

		a_dot.append(Lambda*eps_A*pow(omega[i],2)*pow(sina,-1)*(Tsdotomega*cosa-(1-eps_I_prime*pow(sina,2))*Tsz) - eps_A*omega[i]*Taz*(1-eps_I_prime*pow(sina,2))*pow(sina,-1)
		)

		phi_dot.append(pow(omega[i],2)*Lambda*eps_A*pow(sina,-1)*(Tsy*cosphi-Tsx*sinphi)+omega[i]*eps_A*pow(sina,-1)*(Tay*cosphi-Tax*sinphi))
	...
	if options.save_fig:
		py.savefig("3D_dotted_variables_"+file_name.rstrip(".txt")+".png")
	...
	if options.save_data:
		write_file=open("dotted_variables_"+file_name,"w+")
		for i in range(len(omega_dot)):
			write_file.write(str(time[i])+" "+str(omega_dot[i])+" "+str(a_dot[i])+" "+str(phi_dot[i])+"\n")
		write_file.close()
