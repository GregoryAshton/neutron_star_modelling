...
the pulsation frequency and spindown. We aim to simule these physical
observables and observe the effect of various timing noise mechanisms. To do
...
To define the transformation to the intertial, we use a rotation matrix 
...
we uniquely define the inital Euler angles. We are free to set the initial
...
A spinning biaxial body free from torques will undergo two motions: the fast
rotation and a slower precession about the symmetry axis. Decomposing the
motion into the Euler angles \citet{Jones2001} demonstrated that: the wobble
angle $\theta$ remains fixed; the azimuthal angle $\phi$ monotonically
increases at the spin frequency $\dot{\phi}$; the body frame precession refers
to the decrease (for oblate bodies) of $\psi$ at the slower precession
frequency $\dot{\psi}$. We begin by testing that our numerical solutions of
equations \eqref{eqn: ODEs} is consistent with this description.
...
$\theta$ should remain constant during this simulation but is instead found to
vary; although the fractional size of the variations are small $\sim 10^{-11}$.
...
short time scales these errors have fractional size $1\times10^{-14}$ and so
...
biaxial body with deformation of $\epsilon_{I3} = 1 \times 10^{-3}$}
...
columnated beam is emitted along the magnetic dipole. In the body frame we
...
An observer sees a pulse from the NA every time the column from the magnetic
...
will occur everytime the magnetic dipole cuts the plane containing the observer
...
$\psi$ rotates the vector in a cone aboout the $\n_{d}$ axis; holding instead
$\psi$ fixed and increasing $\phi$ sweeps the vector about a cone centered
...
\caption{Diagrams depiciting 2D projections of the cones swept out by the
...
cone oscillates sinusoidally between $\theta+\chi$ and $\theta-\chi$
...
thehalf angle of the dipole cone increases.
\end{itemize}•
...
torque this model cannoot not spin down. We can however interpret these results
...
\end{figure}
The results show that the precession induces a sinusoidal variation on the
...
spindown of observed pulsars. Second order Taylor expansions are fittied to
...
free precession. As the magnetic dipole
performs a slow rotation about the deformation axis during half the cycle it
counter-rotates and the other half it corotates with the rapid rotation about
the angular momentum vector. The result is a symmetric modulation of the
spindown about zero.
...
$\Phi_{O}, \Theta_{O}$ and then assume the emmision region follows
...
\label{eqn: Amplitude}
\end{equation}•
where $\tilde{\Phi}=\mod_{2\pi}(\Phi)-\Phi_{O}$ and similarly for
$\tilde{\Theta}$. Computing the angles $\Phi$ and $\Theta$ from numerical
solutions of the original ODEs and using the above emmision model gives 
...
\caption{Amplitude variation using a 2D Gaussian emmision.}
