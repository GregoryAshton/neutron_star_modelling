	...
	if Option_Dictionary.has_key('3D'):
		...
		if Option_Dictionary.has_key('split'):
			...
			for j in range(0,len(values),2):
				...
				# Remove points from the back of the sphere
				front_x = [] ; front_y=[] ; front_z=[]
				back_x = [] ; back_y=[] ; back_z=[]
				for i in range(len(x)):
					
					if -180 < py.arctan2(y[i],x[i])*180/pi < -50 :
						front_x.append(x[i])
						front_y.append(y[i])
						front_z.append(z[i])
					elif 140 < py.arctan2(y[i],x[i])*180/pi < 180 :
						front_x.append(x[i])
						front_y.append(y[i])
						front_z.append(z[i])
					else : 
				...
				ax.plot3D(front_x,front_y,front_z,".",alpha=1.0,color=colors[j/2])	
				ax.plot3D(back_x,back_y,back_z,".",alpha=0.1,color=colors[j/2])		
		else :	
			# Trajectory of path
