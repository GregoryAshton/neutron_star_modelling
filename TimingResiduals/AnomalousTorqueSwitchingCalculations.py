...
if "SingleSwitch" in sys.argv:
    ...
    a0=a0, T=T, AnomTorque=False , upsilon=upsilon, n=500000, error=1e-10,
    ...
    F1 = -(2/3.0) * (R * omega0**3 / c) * epsA * np.sin(np.radians(chi0-a0))**2
    print "dPhi_TS = {:1.2e}".format(upsilon * F1 * T**2 * np.pi/(32 * 2 * np.pi))

if "B1828" in sys.argv:
    epsI = 1.8e-8 # np.logspace(-12, -10, 100)
    chi = 60
    upsilon = 1e-2 * 0.71
    ...
    print "DeltaBeta = {:1.2e}".format(DeltaBeta)
    print "EpsilonA = {}".format(epsA)
    print "dPhi_FP = {}".format(DeltaBeta * np.cos(np.radians(chi))/np.sin(np.radians(chi)))
    print "dPhi_TS = {:1.2f}".format(upsilon * nudot0 * T**2 * np.pi/32.)
    print "Timing residual TS: {}".format(upsilon * nudot0 * T**2 * np.pi/(32. * 2 * np.pi))
