...
def Save_Figure(file_name,type_of_plot,format_type=".png"):
	plot_file_name = type_of_plot+"_"+file_name.rstrip(".hdf5")+format_type
...
def ThreeD_Plot_Cartesian(file_name,Option_Dictionary):
	...
	# Import the data in components x,y,z we use generic names incase the EBF axis is used
	#(time,x,y,z) = File_Functions.Import_Data(file_name,max_n)
	f = File_Functions.Read_File(file_name)
	time = f['time'].value
	x = f['time'].value
	...
	time = time[start:stop] ; x = x[start:stop] ; y = y[start:stop] ; z = z[start:stop]
...
def Spherical_Plot_Transform(file_name,Option_Dictionary):
	...
	if Option_Dictionary.has_key('no_omega'):
		...
		if abs(phi_prime[-1])>1000:
			...
			(phi_prime_scaled , scale) = Useful_Tools.Sort_Out_Some_Axis(phi_prime)
