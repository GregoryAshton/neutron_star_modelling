...
Contains the functions associated with Non-Linear dynamics such as Paramater space plot and Correlation dimension
...
def Parameter_Space_Plot(file_name,Option_Dictionary={}):
	...
	where the axis is the projection of "z" along which the 2D plot is made following
	...
	(time,omega_dot,a_dot,phi_dot)= Dotted_Variable(file_name,nmax)
	...
	# Plot the 'shadows'
	ax.plot(omega_dot, a_dot, min(phi_dot),zdir='z',color="k",alpha=0.2,lw=0.8)
	...
	if "close" in Option_Dictionary:
		...
		try :
			(axis,xmin,xmax,ymin,ymax) = Option_Dictionary['close'].split("/")
		except ValueError :
			print " ERROR: close should have value 'axis/xmin/max/ymin/ymax' no plot will be produces"
			return
		# Remove the axis 
		...
		try : 
			...
			print "x and y lims are not properly defined these should be floats. Using full range instead"
...
def Correlation_Sum(file_name,Option_Dictionary={}):
	...
	try :
		R_min=float(Option_Dictionary['R_min']) ; R_max=float(Option_Dictionary['R_max']) ; Number=int(Option_Dictionary['Number'])
	except KeyError :
		print "You must specify R_min,R_max (ints) and Number in the Option_Dictionary"
		return
	...
	for R in R_list:
		...
		if sumV==0.0: 
			...
			if C < 10.0/float(N) :
				...
				print "More than 10 percent of all points in the test sphere of radius %s, this data will not be used in calculating the best fit" % R
				print sumV , N
				lnC_outsiders_list.append(py.log(C))
				lnR_outsiders_list.append(py.log(R))
			else : 
	...
	ax.plot(x_fit,y_fit,color="b",label="$\ln(c)="+str(round(f_p[0],2))+"\ln(R)  +"+str(round(f_p[1],2))+"$")
	ax.plot(lnR_list,lnC_list,"o",color="r",label="Data used in fit")
	ax.plot(lnR_outsiders_list,lnC_outsiders_list,"x",color="r",label="Data not used in fit")

	# Give upper and lower bounds on C(R)
	ax.axhline(py.log(0.1),ls="--",color="k",alpha=0.6)
	ax.axhline(py.log(10.0/float(N)),ls="--",color="k",label="Bounds on $\ln(C(R))$",alpha=0.6)
...
def Dotted_Variable(file_name,nmax):
	""" Function takes the file imports it and produces the dotted variables as lists """

	(time,x,y,z)= File_Functions.Import_Data(file_name,nmax)
...
#	beta=Physics_Functions.Beta_Function(epsI,epsA,chi)
